...
const { NotFoundError, BadRequestError } = require("./error.handle");
...
const checkProductsExist = async (products) => {
  ...
  const existingProducts = await _Product.find({
    _id: { $in: productIds },
  });
  ...
  if (notFoundProductIds.length > 0) {
    throw new NotFoundError(
      `Không tìm thấy sản phẩm với các _id: ${notFoundProductIds.join(", ")}`
    );
  }
